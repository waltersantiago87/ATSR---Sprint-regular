...
# -*- coding: utf-8 -*-
"""
ATSE – Formulário individual por integrante (Streamlit)
------------------------------------------------------
...
Como executar localmente (com Python instalado):
    ...
    streamlit run atse_form_app.py

Como empacotar em .EXE (Windows) para quem não tem VS Code / Python:
    pip install pyinstaller
    pyinstaller --onefile --add-data "atse_form_app.py;." --name ATSE_Form atse_form_app.py
(Depois, execute o ATSE_Form.exe gerado em dist/.)
"""
...
# Subgrupos atualizados (apenas CC e DS)
SUBGRUPOS = {
    "CC": [
        "Breno Santiago",
        "Filipe Correia",
...
def pagina_organizador():
    ...
    if df is None or df.empty:
        st.warning("Ainda não há respostas salvas (respostas_ATSE.csv).")
        return
